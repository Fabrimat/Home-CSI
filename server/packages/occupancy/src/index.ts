 ...
 * `occupancy_states` hypertable. See docs/architecture.md ("Motion, not
 * people") for why this is a latched state machine, not a per-window
 * classifier, and stateMachine.ts for the machine itself.
 *
 * Contract: see packages/cli/CONTRACTS.md ("occupancy"). Owned by brief B4.
 */
export { runOccupancyPipeline, runOccupancyPipelineCore } from './pipeline.js';
export type {
  FeatureRow,
  FeatureSource,
  OccupancyDetails,
