...
--
-- Why this migration turns compression OFF on both tables, does its work,
-- and turns it back ON:
--
--   Migration 003 enables TimescaleDB compression on csi_records and
--   heartbeats, and a compression-enabled hypertable rejects most of the DDL
--   below. VERIFIED against a live timescale/timescaledb:2.17.2-pg17, three
--   deploys in a row, each one finding a new statement it would not accept:
--
...
--
--     CREATE UNIQUE INDEX ... ON csi_records (...);
--     -> operation not supported on hypertables that have compression
--        enabled
--
--   Adding the column with a default and keeping it works around only the
--   first of those. Rather than keep discovering restrictions one deploy at a
--   time, this migration removes the precondition: with compression disabled
--   these are plain hypertables and every statement below is ordinary DDL.
--
--   Note the restriction is on the table's compression SETTING, not its
--   contents — "the hypertables are empty" buys no exemption. Being empty is
--   what makes disabling compression legal, though: `SET (compress = false)`
--   is refused if any chunk is already compressed. A future migration
--   needing this on a table with compressed chunks would have to
--   decompress_chunk() them first, which is not free.
--
--   The compression settings and policy intervals re-applied at the bottom
--   MUST stay identical to migration 003's. They are duplicated, and nothing
--   enforces the agreement: change one, change the other. The retention
--   policies from 003 are deliberately not touched here — disabling
--   compression does not disturb them.
--
...
SELECT remove_compression_policy('csi_records', if_exists => true);
SELECT remove_compression_policy('heartbeats', if_exists => true);
ALTER TABLE csi_records SET (timescaledb.compress = false);
...
-- Restore exactly what migration 003 set up. Keep in sync with it.
ALTER TABLE csi_records SET (
  timescaledb.compress,
  timescaledb.compress_segmentby = 'node_id',
  timescaledb.compress_orderby = 'time DESC'
);
SELECT add_compression_policy('csi_records', INTERVAL '1 day');
...
SELECT add_compression_policy('heartbeats', INTERVAL '3 days');
